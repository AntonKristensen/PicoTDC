...
plot!(fiddipoints,  0.1 * fiddiamount[1] *  pdf(Normal(fiddimedi[1], fiddispread[1]), fiddipoints), label="50ps fit", color=:blue)
histogram!(cuthunnidata[:,1], bins=hunnipoints, color=:red, label="100ps", alpha=0.50)
plot!(hunnipoints,  0.1 * hunniamount[1] *  pdf(Normal(hunnimedi[1], hunnispread[1]), hunnipoints), label="100ps fit", color=:red)
title!("Monoenergetic Neutron Spectrum")
xlabel!("Energy (MeV)")
ylabel!("Counts")
