...
function findincidentenergies(matcharray, first, second, distance, angle, sizecorrection; timeuncertainty=0)
    ...
    a = createdprotons.^2 .- (cos(angle)^2 * (createdprotons.^2 .+ 2 * createdprotons * 938.27))
    b = 2 * 938.27 * createdprotons.^2
    c = 938.27^2 * (createdprotons.^2 .+ (cos(angle)^2 * (createdprotons.^2 .+ 2 * 938.27 * createdprotons)))

    # Here is the exact solution to the 2nd order polynomial
    #solution = - 938.27 * (1 .+ sqrt.(cos(angle)^4 * (1 .+ 4 * 938.27 ./ createdprotons .+ 4 * 938.27^2 ./ (createdprotons).^2 ))) ./ (1 .- cos(angle)^2 * (1 .+ 2 * 938.27 ./ createdprotons)) .- 938.27
    ...
    incidenthadrons = (-b .- sqrt.(b.^2 .- 4 .* a .* c)) ./ (2 .* a) .- 938.27 
    #println(incidenthadrons)
