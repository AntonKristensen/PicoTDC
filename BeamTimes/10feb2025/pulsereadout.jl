...
function dataanalyzer(datasetnumber; plotting=false)
    ...
    if plotting
        ...
        plot!(4 * collect(0:length(testtrace)-1), testtrace, color=:blue, alpha=1, linewidth=3, legend=false, label="First detector")
        title!("Proton pulses")
        xlabel!("Time (ns)")
        ylabel!("ADC units")
        savefig("plots/traceplot.svg")
        display(peakplot)
    end
    ...
    if plotting
        ...
        display(maxplot)
        sleep(1)
        intervalplot = histogram(intervals, bins=1000, legend=:false)
        #title!(files[datasetnumber][6:end-4])
        title!("Peak intervals")
        xlabel!("Time between peaks (ns)")
        ylabel!("Counts")
        savefig("plots/peakintervals.svg")
        display(intervalplot)
        sleep(1)

        doublehistogram = histogram2d(widths[widths .< 10], heights[widths .< 10], bins=(500,500), legend=:false)
        title!(files[datasetnumber][6:end-4])
        xlabel!("Pulse width")
        ylabel!("Pulse height")
        savefig("plots/doublehistogram.svg")
    ...
    if plotting
        timehist = histogram(filteredtimeintervals, bins=50, legend=:false)
        title!("Pulse intervals")
        xlabel!("Time between intervals")
        ylabel!("Counts")
        savefig("plots/timehist.svg")
        display(timehist)
    end
...
title!("Cyclotron time intervals")
xlabel!("Measurement number")
ylabel!("Cyclotron pulse time (ns)")
savefig("plots/resultplot.svg")
display(resultplot)
